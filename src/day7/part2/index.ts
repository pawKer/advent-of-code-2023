...
type Hand = {points: number, bet:number, hand: string, jpoints: number}
...
inputData.forEach((row) => {
    ...
    if(hand.includes('J')) {
        Object.keys(pokerCards).filter(c => c !== 'J').forEach(k => {
            const newCards = hand.replaceAll('J', k).split('')
            const handTypePoints = calculateHandTypePoints(newCards);
    ...
    handPoints.push({points: hand.includes('J') ? maxScore : totalPoints, jpoints: totalPoints, bet, hand})
})

const handCompareFn = (a: Hand, b: Hand) => {
    if(a.points === b.points && a.hand.includes('J') && b.hand.includes('J')) {
        return a.jpoints - b.jpoints
    } 
